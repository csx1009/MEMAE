...
from .vim_u import PatchEmbed3D, create_block, _init_weights, PatchEmbedUniversial
...
class MaskedAutoencoderVim(nn.Module):
    ...
    def patchify_3d(self, volume):
        ...
        """
        p = self.patch_embed.patch_size  # Patch size
        c = self.in_chans
        h, w, d = self.patch_embed.grid_size
        # l = h = w = volume.shape[2] // p  # Since volumes have the same dimension. Possible limitation??
        x = volume.reshape(shape=(volume.shape[0], c, h, p[0], w, p[1], d, p[2]))
        x = torch.einsum('nclrhpwq->nlhwrpqc', x)
        x = x.reshape(shape=(volume.shape[0], h * w * d, np.prod(p) * c))
        return x
...
def mae_3d_vim_small_patch4_rms_bimambav2_rope_residual(**kwargs):
    model = MaskedAutoencoderVim(
        patch_size=[4, 4, 5], embed_dim=384, depth=12,
        decoder_embed_dim=192, decoder_depth=8,
        rms_norm=True,
        residual_in_fp32=True,
        fused_add_norm=True,
