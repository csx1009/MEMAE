...
from .models_vim import MambaVisionLayer

from .util.helpers import PatchEmbed, get_3d_sincos_pos_embed


class MaskedAutoencoderMambavision_3D(nn.Module):
    """ Masked Autoencoder with Mambavision backbone
    """
    ...
    def __init__(self, img_size: tuple, patch_size: tuple, in_chans,
        ...
        self.pos_embed = nn.Parameter(torch.zeros(1, total_num_patches + 1, embed_dim),requires_grad=False)
        ...
        for i in range(len(depths)):
            ...
            level = MambaVisionLayer(dim=dim,
                                     depth=depths[i],
        ...
        #           mlp_ratio=mlp_ratio,
        #           transformer_blocks=list(range(decoder_depth // 2, decoder_depth)),
        #           ).to('cuda')
        #     for i in range(decoder_depth)])
        decoder_depths = [1, 2, 2]
        decoder_num_heads = [4, 8, 16]
        decoder_window_size = [8, 8, 14]
        decoder_dim = decoder_embed_dim
        decoder_drop_path_rate = 0.3
        decoder_layer_scale = 1e-5
        decoder_dpr = [x.item() for x in torch.linspace(0, decoder_drop_path_rate, sum(decoder_depths))]
        self.decoder_blocks = nn.ModuleList()
        for i in range(len(decoder_depths)):
            conv = True if (i == 1 or i == 2) else False
            level = MambaVisionLayer(dim=decoder_dim,
                                     depth=decoder_depths[i],
                                     num_heads=decoder_num_heads[i],
                                     window_size=decoder_window_size[i],
                                     mlp_ratio=mlp_ratio,
                                     conv=conv,
                                     drop=0,
                                     attn_drop=0,
                                     drop_path=decoder_dpr[sum(decoder_depths[:i]):sum(decoder_depths[:i + 1])],
                                     downsample=(i < 3),
                                     layer_scale=decoder_layer_scale,
                                     layer_scale_conv=None,
                                     transformer_blocks=list(range(decoder_depths[i] // 2 + 1, decoder_depths[i]))
                                     if decoder_depths[i] % 2 != 0 else list(range(decoder_depths[i] // 2, decoder_depths[i])),
                                     )
    ...
    def unpatchify(self, x):
        ...
        p = self.patch_size
        h, w, d = self.num_patches
        assert h * w * d == x.shape[1], f'''size error: ({h}, {w}, {d}) vs {x.shape[1]}'''
...
mae_vit_192 = mae_vit_base_dec192d8b
