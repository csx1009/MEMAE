...
try:
    from .utils.layer_norm import RMSNorm, layer_norm_fn, rms_norm_fn
except ImportError:
...
class PatchEmbed3D(nn.Module):
    ...
    def __init__(self, volume_size: tuple, patch_size: tuple, in_chans=3, embed_dim=768, norm_layer=None, flatten=True):
        super().__init__()
...
class PatchEmbedUniversial(nn.Module):
    ...
    def __init__(self, img_size=224, in_chans=3, embed_dim=768, norm_layer=None, if_3d=False):
        ...
        self.num_patches = self.img_size[0] * self.img_size[1] if not if_3d else self.img_size[0] * self.img_size[1] * \
                                                                                 self.img_size[2]
        self.if_3d = if_3d
...
def vim_small_patch16_224_bimambav2_final_pool_mean_abs_pos_embed_div2(pretrained=False, **kwargs):
    model = VisionMamba(
        patch_size=16, embed_dim=384, depth=12, rms_norm=True, residual_in_fp32=True, fused_add_norm=True,
        final_pool_type='mean',
        if_abs_pos_embed=True, if_rope=True, if_rope_residual=True, bimamba_type="v2", if_cls_token=False,
...
def vim_small_patch8_224_bimambav2_final_pool_mean_abs_pos_embed_div2(pretrained=False, **kwargs):
    model = VisionMamba(
        patch_size=8, stride=8, embed_dim=384, depth=12, rms_norm=True, residual_in_fp32=True, fused_add_norm=True,
        final_pool_type='mean',
        if_abs_pos_embed=True, if_rope=True, if_rope_residual=True, bimamba_type="v2", if_cls_token=False,
        if_devide_out=True, use_middle_cls_token=False, **kwargs)
    model.default_cfg = _cfg()
...
def vim_small_patch4_224_bimambav2_final_pool_mean_abs_pos_embed_div2(pretrained=False, **kwargs):
    model = VisionMamba(
        patch_size=4, stride=4, embed_dim=384, depth=12, rms_norm=True, residual_in_fp32=True, fused_add_norm=True,
        final_pool_type='mean',
        if_abs_pos_embed=True, if_rope=True, if_rope_residual=True, bimamba_type="v2", if_cls_token=False,
        if_devide_out=True, use_middle_cls_token=False, **kwargs)
    model.default_cfg = _cfg()
